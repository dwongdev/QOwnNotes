...
- Fixed percent-encoded local Markdown links not showing the pointer cursor or opening
  with <kbd>Ctrl</kbd> + click in the note editor
...
- Fixed HTML `<sub>` and `<sup>` tags not rendering as subscript and superscript in
  the QLiteHtml note preview
  (for [#3728](https://github.com/pbek/QOwnNotes/issues/3728))
...
- Added a `Username` field to the LanguageTool settings so LanguageTool Premium
  ...
  (for [#3718](https://github.com/pbek/QOwnNotes/issues/3718), thank you, @gregoreficint)
...
- The **Script repository** dialog now links to available script changelogs and shows
  ...
  a script (for [#3709](https://github.com/pbek/QOwnNotes/issues/3709))
...
- Opening the search bar in the **note text edit**, the **preview**, or the
  **QLiteHTML preview** now keeps the current horizontal and vertical scroll position
  ...
  view away from where you were reading (for [#3541](https://github.com/pbek/QOwnNotes/issues/3541))
...
- Improved lookup of the `Bookmarks` and `Commands` notes so they are now found
  ...
  where it is** (for [#2518](https://github.com/pbek/QOwnNotes/issues/2518))
...
- Added optional **LanguageTool grammar and style checking** alongside the existing Sonnet
  ...
  - For more information on the LanguageTool integration, see the new documentation page at
    [LanguageTool](https://www.qownnotes.org/editor/languagetool.html)
...
- Fixed Homepage bookmark auto-suggestions to also match tokens from full Markdown bookmark
  entries (tags and trailing text after the link) (for [#3484](https://github.com/pbek/QOwnNotes/issues/3484))
- Fixed merged bookmark entries keeping a shorter link label when a longer label
  exists for the same URL (for [#3484](https://github.com/pbek/QOwnNotes/issues/3484))
...
- Fixed angle-bracket content inside code blocks (also 4-Spaces fence) and inline code
  ...
  - This also fixes regex preview issues (for [#2952](https://github.com/pbek/QOwnNotes/issues/2952))
