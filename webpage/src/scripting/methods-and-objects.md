...
You can also add highlighting rules with custom foreground/background colors
and font styles, instead of being limited to the predefined highlighting states.
This allows you to define your own color schemes for custom syntax patterns.
...
::: tip
You can combine a predefined `state` with custom format properties.
The custom properties will override the state's defaults. Use state `-1`
(`NoState`) if you only want to use custom formatting.
:::
